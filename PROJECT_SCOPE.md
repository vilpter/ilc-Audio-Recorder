# Project: Headless Dual-Mono Audio Archiving System
...
#### B. Systemd Service
- [ ] Create `audio-archiver.service` file
- [ ] Enable auto-start on boot
- [ ] Configure restart policy (always/on-failure)
- [ ] Set up logging to `/var/log/audio-archiver/app.log`
...
2. **Configure headless access** (use Pi Imager "Settings" gear icon):
   - Enable SSH
   - Set hostname (e.g., `audio-archiver`)
...
5. Connect via SSH: `ssh pi@audio-archiver.local` (or use IP address)
...
cd ~
# [Transfer audio-archiver directory]
...
sudo cp audio-archiver.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable audio-archiver
sudo systemctl start audio-archiver
...
├── audio-archiver.service   # ❌ Systemd service file
